  ...
  | TypeDec (s,t) -> 
      let entry = tenv s in
         match entry with
           ...
           | _ -> raise (Err s)
and trans_init ast nest tenv env = match ast with
    InitVar (s,e) -> let exp_code = trans_exp e nest env in
let entry = env s in
    let var_code = (match entry with
                            VarEntry {offset=offset; level=level; ty=_} -> 
                                  "\tmovq %rbp, %rax\n" 
                                ^ nCopyStr (nest-level) "\tmovq 16(%rax), %rax\n"
                                ^ sprintf "\tleaq %d(%%rax), %%rax\n" offset
                          | _ -> raise (No_such_symbol s)) in
      exp_code
    ^ var_code
    ^ "\tpopq (%rax)\n"
(* 文の処理 *)
and trans_stmt ast nest tenv env = 
                 ...
                 match ast with
                  ...
                  | CallInc (v) -> 
                                    ...
                                    ^ "\taddq $1,(%rax)\n"
                  (* initializeのコード *)
                  | CallInit (v,e) -> trans_exp e nest env
                                    ^ trans_var v nest env
                                    ^ "\tpopq (%rax)\n"
                  (* 手続き呼出しのコード *)
                  | CallProc (s, el) -> 
                  ...
                  | While (e,s) -> let (condCode, l1) = trans_cond e nest env in
                                     let l2 = incLabel() in
                                       ...
                                       ^ sprintf "L%d:\n" l1
                  (* do文のコード *)
                  | Do (s,e) -> let (condCode,l1) = trans_cond e nest env in
                                let l2 = incLabel() in
                                    sprintf "L%d:\n" l2
                                  ^ trans_stmt s nest tenv env
                                  ^ condCode
                                  ...
                                  ^ sprintf "L%d:\n" l1
                  (* for文のコード *)
                  | For (v,e1,e2,s) -> trans_exp e2 nest env
...
and trans_exp ast nest env = match ast with
                  ...
                  | CallFunc("!",  arg::_) -> 
                                             ...
                                             ^ "\tnegq (%rsp)\n"
                  (* ++のコード *)
                  | CallInc v ->             trans_var v nest env
                                           ^ "\tmovq (%rax),%rbx\n"
                                           ^ "\taddq $1,(%rax)\n"
                                           ^ "\tpushq %rbx\n"
                  (* +=のコード *)
