     ...
     FuncDec (s, l, rlt, Block (dl,_)) ->
         ...
         let env' = update s (FunEntry
                                 {formals=
                                  ...
                                  result=create_ty rlt tenv; level=nest+1}) env in
         (tenv, env', addr)
    (* 変数宣言の処理 *)
    | VarDec (t,s) -> (tenv,
...
and type_init ast (nest,addr) tenv env =
  match ast with
    InitVar (s,e) -> (tenv,update s(VarEntry {ty=INT;offset=addr-8;level=nest}) env,addr-8)
  | _ -> raise (Err "internal error")

and type_inits il nest tenv env =
  List.fold_left (fun (tenv,env,addr) d -> type_init d (nest,addr) tenv env) (tenv,env,0) il
...
and type_stmt ast env = 
       match ast with
          ...
          | CallProc ("+=",[v;arg]) -> if ((type_exp v env) !=INT && (type_exp arg env) != INT) then raise (TypeErr "type error 3")
          ...
          | Do (_,e) -> type_cond e env
          | For (v,e1,e2,s) ->  if ((type_var v env) != (type_exp e1 env)) then  raise (TypeErr "type error 4")
          | CallAdd (v,e) -> if((type_var v env) != (type_exp e env)) then raise (TypeErr "type error 4")
          | CallInc (v) -> if((type_var v env) != INT) then raise (TypeErr "type error 4")
          | CallInit (v,e) -> if((type_var v env) != (type_exp e env)) then raise (TypeErr "type error")
          | NilStmt -> ()
...
and type_exp ast env = 
        match ast with
          ...
          | CallFunc ("^",[left; right]) ->
             (check_int (type_exp left env); check_int(type_exp right env); INT)
          | CallInc (s) ->  (type_var s env;INT)
          | CallAdd (s,e) ->
             (type_var s env;check_int(type_exp e env);INT)
          | CallFunc ("!", [arg]) -> 
