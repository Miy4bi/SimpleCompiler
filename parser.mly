...
%token PLUS MINUS TIMES DIV MOD POW INC ADD LB RB LS RS LP RP ASSIGN SEMI COMMA TYPE VOID
%type <Ast.stmt> prog
...
inits: inits init { $1@$2 }
     | 	  { [] }
     ;

init : INT vexps SEMI  { List.map (fun (x,y) -> InitVar(x,y)) $2}
     ;

vexps: vexps COMMA ID ASSIGN expr {$1@[($3,$5)]}
     | ID ASSIGN expr             {[($1,$3)]}
     ;
...
stmt : ID ASSIGN expr SEMI    { Assign (Var $1, $3) }
     ...
     | ID ADD expr SEMI       { CallAdd (Var $1,$3) }
     | ID INC SEMI            { CallInc (Var $1) }
     | IF LP cond RP stmt     { If ($3, $5, None) }
     | IF LP cond RP stmt ELSE stmt
                              { If ($3, $5, Some $7) }
     | WHILE LP cond RP stmt  { While ($3, $5) }
     | DO stmt WHILE LP cond RP { Do ($2,$5) }
